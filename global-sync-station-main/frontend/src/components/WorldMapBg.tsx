...
export function WorldMapBg({ variant = 'darker' }: { variant?: 'darker' | 'brighter' }) {
  ...
  const svgOpacity = brighter ? 0.45 : 0.3;
  const particleOpacity = brighter ? 0.78 : 0.4;
  const vignetteOpacity = brighter ? 0.04 : 0.06;
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <svg viewBox="0 0 1000 500" className="absolute inset-0 w-full h-full" style={{ opacity: svgOpacity }}>
        <defs>
          <radialGradient id="globe-glow" cx="50%" cy="50%" r="50%">
            ...
            <stop offset="100%" stopColor={brighter ? '#0ea5a4' : 'oklch(0.85 0.18 78)'} stopOpacity="0" />
          </radialGradient>
          <pattern id="dots" width="6" height="6" patternUnits="userSpaceOnUse">
            <circle cx="1" cy="1" r="0.9" fill={brighter ? '#00d9ff' : 'oklch(0.85 0.18 78)'} />
          </pattern>
          <mask id="continents">
            {/* North America */}
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1000 500" preserveAspectRatio="none" style={{ opacity: brighter ? 0.55 : 0.4 }}>
        {[
          ...
          <g key={i}>
            <circle cx={x} cy={y} r="3" fill={brighter ? '#00d9ff' : 'oklch(0.9 0.2 80)'} style={{ opacity: particleOpacity }} />
            <circle cx={x} cy={y} r="6" fill="none" stroke={brighter ? '#0ea5a4' : 'oklch(0.9 0.2 80)'} strokeOpacity={brighter ? 0.55 : 0.5} className="animate-radar" style={{transformOrigin: `${x}px ${y}px`}} />
          </g>
        ))}
      </svg>
        ...
        {particles.map((p, i) => (
          <div
            ...
            style={{
              ...
              boxShadow: brighter ? '0 0 8px rgba(0,217,255,0.45)' : '0 0 6px oklch(0.85 0.18 78)',
